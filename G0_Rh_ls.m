function [G0_Rh_ls] = G0_Rh_ls(T, P_tot, X_i, P_units)
%
...
mask1 = (T>298) .* (T<=2233);      % solid

% solid
G0_Rh_ls = mask1.*(- 6668.85243 + 111.597530*T - 6.723688000e-03*T.^(2)- 16945.2000*T.^(-1)+ 3.779546667e-07*T.^(3) - 20.8112160 *T.*log(T));

% now convert units to eV per Ga2O
G0_Rh_ls = G0_Rh_ls/(avo*q);   % eV/Ga2O molecule

% Now take Ptot and Xi into account.  
G0_Rh_ls = G0_Rh_ls + kB_eV*T.* ( log(P_tot/P_ref) + log(X_i));

% set any that are zero becasue of masking to infintiy so it produces an
% obvious error that can be seen 
G0_Rh_ls(G0_Rh_ls==0) = Inf;

end


% View Data  Rh     Units:  T(K) P(atm) Energy(J) Quantity(mol) 
% Name: Rhodium
% 
%   G(T) J/mol - 1 atm  
% 
...
% 
% S1         1 - 6668.85243          + 111.597530     T     - 6.723688000E-03 T^ 2   298 - 2233  
% S1         1 - 16945.2000     T^-1 + 3.779546667E-07 T^ 3 - 20.8112160     T ln(T) 298 - 2233  
% L1         2 - 7042.61295          + 260.787191     T     - 41.8400000     T ln(T) 2233 - 3970 
% G1         3 550625.658            - 44.9682483     T     - 21.0120480     T ln(T) 295 - 300   
% ____________ _____________________ ______________________ ________________________ ___________ 
% 
