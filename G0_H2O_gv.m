function [G0_H2O_gv] = G0_H2O_gv(T, P_tot, X_i, P_units)
%
...
mask1 = (T>298) .* (T<=1100);      % gas
mask2 = (T>1100) .* (T<=4000);      

% vapor
G0_H2O_gv = mask1.*(- 255475.808 + 13999.6597*T.^(-1) - 15.1731427*T - 7.474858139e-3*T.^(2) + 9.205931575e-8*T.^(3) + 1107.27182*log(T) - 25.7816397*T.*log(T));
G0_H2O_gv = G0_H2O_gv + mask2.*(152152.281 - 12075583.2*T.^(-1) + 164.817017*T - 8.054003823e-5*T.^(2) + 5947.37003*T.^(0.5) - 83128.2757*log(T) - 53.1457895*T.*log(T));

% now convert units to eV per Ga2O
G0_H2O_gv = G0_H2O_gv/(avo*q);   % eV/Ga2O molecule

% Now take Ptot and Xi into account.  
G0_H2O_gv = G0_H2O_gv + kB_eV*T.* ( log(P_tot/P_ref) + log(X_i));

% set any that are zero becasue of masking to infintiy so it produces an
% obvious error that can be seen 
G0_H2O_gv(G0_H2O_gv==0) = Inf;
