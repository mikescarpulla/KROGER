function [G0_IrO2_ls] = G0_IrO2_ls(T, P_tot, X_i, P_units)
%
...
%
% FAQ4: why keep the Xi for condensed phases?  so we can do mixtures, and
% to keep the input syntax the same.  % FAQ4: why keep the X_i for condensed phases?  You can thus use it to make an ideal solution of multiple phases.  You could make a function that adds a few phases and adds excess entropy and excess enthalpy to represent an alloy.... 
%
...
G0_IrO2_ls = zeros(size(T));

% solid
mask1 = (T>0) .* (T<=800);      %logical array same size as T with ones where true and 0 where false
mask2 = (T>800) .* (T<=1400);   
G0_IrO2_ls = G0_IrO2_ls + mask1.*(- 261863.998 + 557000*T.^(-1) - 1430.8*T.^(0.5) + 613.806359*T - 93.27*T.*log(T));
G0_IrO2_ls = G0_IrO2_ls + mask2.*(- 265858.066 + 420.004152*T - 5.5e-3*T.^(2) - 70.31*T.*log(T));
...
% % % % % % 
% % % View Data  IrO2     Units:  T(K) P(atm) Energy(J) Quantity(mol) 
...
% % % 
% % % S1         1 - 261863.998             + 613.806359     T       + 557000.000     T^-1  298 - 800  
% % % S1         1 - 1430.80000     T^0.5   - 93.2700000     T ln(T)                        298 - 800  
% % % S1         2 - 265858.066             + 420.004152     T       - 5.500000000E-03 T^ 2 800 - 1400 
% % % S1         2 - 70.3100000     T ln(T)                                                 800 - 1400 
% % % G1         3 198977.352               + 161.679228     T       + 218550.000     T^-1  298 - 900  
% % % G1         3 - 339.760000     T^0.5   - 61.6600000     T ln(T)                        298 - 900  
% % % G1         4 198199.809               + 116.765974     T       - 1.101000000E-03 T^ 2 900 - 2000 
% % % G1         4 - 56.4100000     T ln(T)                                                 900 - 2000 
% % % ____________ ________________________ ________________________ ______________________ __________ 
% % % 
