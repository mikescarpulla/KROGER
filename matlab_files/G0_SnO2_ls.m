function [G0_SnO2_ls] = G0_SnO2_ls(T, P_tot, X_i, P_units)
%
...
mask1 = (T>298) .* (T<=1903);      % solid
mask2 = (T>1903) .* (T<=4400);      % liquid.  

% solid
G0_SnO2_ls = G0_SnO2_ls + mask1.*(- 608087.742 + 1111933.50*T.^(-1) + 474.985631*T -3.681085E-03*T.^(2) -1.281666667E-10*T.^(3) -76.041*T.*log(T));

% liquid
G0_SnO2_ls = G0_SnO2_ls + mask2.*(-600556.627 + 584.839002*T -92.0*T.*log(T));


% now convert units to eV per Ga2O
G0_SnO2_ls = G0_SnO2_ls/(avo*q);   % eV/Ga2O molecule

% Now take Ptot and Xi into account.  
G0_SnO2_ls = G0_SnO2_ls + kB_eV*T.* log(X_i);

% set any that are zero becasue of masking to infintiy so it produces an
% obvious error that can be seen 
G0_SnO2_ls(G0_SnO2_ls==0) = Inf;
