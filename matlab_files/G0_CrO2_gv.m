function [G0_CrO2_gv] = G0_CrO2_gv(T, P_tot, X_i, P_units)
%
...
% liquid
G0_CrO2_gv = mask1.*(-113882.783  +10.6999127*T -1.550250000e-02*T.^(2) +245800.000*T.^(-1) +2.429833333e-06*T.^(3) -39.5260000*T.*log(T));
G0_CrO2_gv = G0_CrO2_gv + mask2.*(-122060.283 +123.922321*T -1.175000000e-04*T.^(2) +932050.000*T.^(-1) -8.016666667e-08*T.^(3) -56.6960000*T.*log(T));

% now convert units to eV per Ga2O
G0_CrO2_gv = G0_CrO2_gv/(avo*q);   % eV/Ga2O molecule

% Now take Ptot and Xi into account.  
G0_CrO2_gv = G0_CrO2_gv + kB_eV*T.* ( log(P_tot/P_ref) + log(X_i));

% set any that are zero becasue of masking to infintiy so it produces an
% obvious error that can be seen 
G0_CrO2_gv(G0_CrO2_gv==0) = Inf;
