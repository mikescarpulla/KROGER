function [G0_GaN_ls] = G0_GaN_ls(T, P_tot, X_i, P_units)
...
G0_GaN_ls = zeros(size(T));

mask1 = (T>298) .* (T<=1773);  % 0 to 1773     logical array same size as T with ones where true and 0 where false

% up to 900 K 
G0_GaN_ls = G0_GaN_ls + mask1.*(-121372.707 + 227.983015*T -4.497800000E-03*T.^2 - 38.0744000.*T.*log(T) );  

% now convert units to eV per GaN
G0_GaN_ls = G0_GaN_ls/(avo*q);   % eV/GaN molecule
