function [G0_As4_gv] = G0_As4_gv(T, P_tot, X_i, P_units)
% T in kelvin
...
% gas1
mask = (T>298) .* (T<=1200);
G0_As4_gv = mask.*(126847.133 + 230.986651* T - 6.485200000e-05* T.^(2) + 256479.200 *T.^(-1) - 82.9394320 *T.* log(T));

% now convert units to eV per Cd
G0_As4_gv = G0_As4_gv/(avo*q);   % eV/Cd
...
G0_As4_gv = G0_As4_gv + kB_eV*T.* ( log(P_tot/P_ref) + log(X_i));

% set any that are zero becasue of masking to infintiy so it produces an
% obvious error that can be seen 
G0_As4_gv(G0_As4_gv==0) = Inf;
G0_As4_gv(isnan(G0_As4_gv)) = Inf;
...
%%
% View Data   As4     Units:  T(K) P(atm) Energy(J) Quantity(mol) 
% Name: As4
% 
%   G(T) J/mol - 1 atm  
% 
...
% 
% G1         1 126847.133            + 230.986651     T       - 6.485200000E-05 T^ 2 298 - 1200 
% G1         1 + 256479.200     T^-1 - 82.9394320     T ln(T)                        298 - 1200 
% ____________ _____________________ ________________________ ______________________ __________ 
% 
