function [G0_RhO2_ls] = G0_RhO2_ls(T, P_tot, X_i, P_units)
%
...
mask1 = (T>298) .* (T<=1000);      % solid

% solid
G0_RhO2_ls = mask1.*(- 250888.185 + 277.070194*T - 1.521302900e-02*T.^(2)- 131609.828*T.^(-1)+ 1.505648333e-06*T.^(3)- 316.646634*T.^(0.5) - 47.7352734 *T.*log(T));

% now convert units to eV per Ga2O
G0_RhO2_ls = G0_RhO2_ls/(avo*q);   % eV/Ga2O molecule

% Now take Ptot and Xi into account.  
G0_RhO2_ls = G0_RhO2_ls + kB_eV*T.* ( log(P_tot/P_ref) + log(X_i));

% set any that are zero becasue of masking to infintiy so it produces an
% obvious error that can be seen 
G0_RhO2_ls(G0_RhO2_ls==0) = Inf;

end


% View Data   RhO2     Units:  T(K) P(atm) Energy(J) Quantity(mol) 
...
% 
% S1         1 - 250888.185             + 277.070194     T     - 1.521302900E-02 T^ 2 298 - 1000 
% S1         1 - 131609.828     T^-1    + 1.505648333E-06 T^ 3 - 316.646634     T^0.5 298 - 1000 
% S1         1 - 47.7352734     T ln(T)                                               298 - 1000 
% G1         2 169504.608               + 136.383579     T     + 690360.000     T^-1  298 - 1500 
% G1         2 - 58.5760000     T ln(T)                                               298 - 1500 
% ____________ ________________________ ______________________ ______________________ __________ 
% 
