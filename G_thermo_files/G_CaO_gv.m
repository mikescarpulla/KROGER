function [G0_CaO_gv] = G0_CaO_gv(T, P_tot, X_i, P_units)
%
...
mask1 = (T>298) .* (T<=1400);      % gas/vapor
mask2 = (T>1400) .* (T<=3800);  
mask3 = (T>3800) .* (T<=6000); 

% gas/vapor
G0_CaO_gv = mask1.*(78867.7875 + 205.751461*T + 1.280263453E-02*T.^(2) - 537947.001*T.^(-1) - 1.712449235E-06*T.^(3) - 10352.2747*log(T) - 61.5491809*T.*log(T));
G0_CaO_gv = G0_CaO_gv + mask2.*(37566279.6 - 406018.798*T + 3.27569158*T.^(2) - 329377998*T.^(-1) - 2.831966034E-05*T.^(3) - 14615548.9*log(T) - 1162.61296*T.^(1.5) + 5256160.29*T.^(0.5) + 48763.2209*T.*log(T));
G0_CaO_gv = G0_CaO_gv + mask3.*(- 12975648.8 + 3153.92066*T + 873451717*T.^(-1) + 2367789.28*log(T) - 178046.562*T.^(0.5) - 282.261951*T.*log(T));


% now convert units to eV per Ga2O
G0_CaO_gv = G0_CaO_gv/(avo*q);   % eV/Ga2O molecule

% Now take Ptot and Xi into account.  
G0_CaO_gv = G0_CaO_gv + kB_eV*T.* ( log(P_tot/P_ref) + log(X_i));

% set any that are zero becasue of masking to infintiy so it produces an
% obvious error that can be seen 
G0_CaO_gv(G0_CaO_gv==0) = Inf;
