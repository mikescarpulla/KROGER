function [G0_AsTe_gv] = G0_AsTe_gv(T, P_tot, X_i, P_units)
% T in kelvin
...
% gas1
mask = (T>298) .* (T<=1900);
G0_AsTe_gv = mask.*(217982.982 - 11.7561026* T - 3.556400000e-04 *T.^(2) - 36.3924320* T.* log(T));

% now convert units to eV per Cd
G0_AsTe_gv = G0_AsTe_gv/(avo*q);   % eV/Cd
...
G0_AsTe_gv = G0_AsTe_gv + kB_eV*T.* ( log(P_tot/P_ref) + log(X_i));

% set any that are zero becasue of masking to infintiy so it produces an
% obvious error that can be seen 
G0_AsTe_gv(G0_AsTe_gv==0) = Inf;
G0_AsTe_gv(isnan(G0_AsTe_gv)) = Inf;
...
%%
% View Data   AsTe     Units:  T(K) P(atm) Energy(J) Quantity(mol) 
% Name: Arsenic Monotelluride
% 
%   G(T) J/mol - 1 atm  
% 
%              G(T)                     G(T)               G(T)                   T(K)       
...
% 
% G1         1 217982.982               - 11.7561026     T - 3.556400000E-04 T^ 2 298 - 1900 
% G1         1 - 36.3924320     T ln(T)                                           298 - 1900 
% ____________ ________________________ __________________ ______________________ __________ 
% 
