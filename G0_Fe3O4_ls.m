function [G0_Fe3O4_ls] = G0_Fe3O4_ls(T, P_tot, X_i, P_units)
%
...
G0_Fe3O4_s1 = zeros(size(T));
G0_Fe3O4_s2 = zeros(size(T));
G0_Fe3O4_s3 = zeros(size(T));
G0_Fe3O4_s4 = zeros(size(T));
G0_Fe3O4_l1 = zeros(size(T));
G0_Fe3O4_ls = zeros(size(T));


% solid1
mask1 = (T>298) .* (T<=848);      %logical array same size as T with ones where true and 0 where false
G0_Fe3O4_s1 = G0_Fe3O4_s1 + mask1.*(-1200277.81  +1282.95850*T -1.901728207E-02*T.^(2) +3621668.52*T.^(-1) +3.967903931E-05*T.^(3)   -3.104596095E-08*T.^(4) -110726059*T.^(-2) -207.930830*T.*log(T) );
mask2 = (T>848) .* (T<=1870);
G0_Fe3O4_s1 = G0_Fe3O4_s1 + mask2.*(-1186819.15  +1260.56231*T  +3621668.52*T.^(-1) -110726059*T.^(-2) -207.930830*T.*log(T) );

% solid 2
mask3 = (T>848) .* (T<=1870);  
G0_Fe3O4_s2 = G0_Fe3O4_s2 + mask3.*(-1185253.60  +1258.71640*T +3621668.52*T.^(-1)  -110726059.*T.^(-2)  -207.930830*T.*log(T) );
mask4 = (T>1870) .* (T<=2500); 
G0_Fe3O4_s2 = G0_Fe3O4_s2 + mask4.*(-1191672.58  +1304.25027*T  -213.384000*T.*log(T));

% solid 3
mask5 = (T>298) .* (T<=848); 
G0_Fe3O4_s3 = G0_Fe3O4_s3 + mask5.*(-1030731.16  +1253.59865*T  -1.901728207E-02*T.^(2) +3621668.31*T.^(-1) +3.967903931E-05*T.^(3) -3.104596095E-08*T.^(4) -110726059.*T.^(-2) -207.930830*T.*log(T) );
mask6 = (T>848) .* (T<=1870); 
G0_Fe3O4_s3 = G0_Fe3O4_s3 + mask6.*(-1017272.51 +1231.20246*T +3621668.52*T.^(-1) -110726059.*T.^(-2) -207.930830*T.*log(T) );

% solid 4
mask7 = (T>848) .* (T<=1870); 
G0_Fe3O4_s4 = G0_Fe3O4_s4 + mask7.*(-1015705.37 +1229.35613*T +3621668.52*T.^(-1) -110726059.*T.^(-2) -207.930830*T.*log(T) );
mask8 = (T>1870) .* (T<=2500); 
G0_Fe3O4_s4 = G0_Fe3O4_s4 + mask8.*(-1022124.35 +1274.89000*T -213.384000*T.*log(T) );

% liquid
mask9 = (T>298) .* (T<=1870); 
G0_Fe3O4_l1 = G0_Fe3O4_l1 + mask9.*(-1047181.60 +1184.88110*T +3621668.52*T.^(-1) -110726059.*T.^(-2) -207.930830*T.*log(T));
mask10 = (T>1870) .* (T<=2500); 
G0_Fe3O4_l1 = G0_Fe3O4_l1 + mask10.*(-1053600.58 +1230.41497*T -213.384000*T.*log(T));


% choose the min Go, thus automatically selecting the right phase for all T's and P's.  
G0_Fe3O4_ls = min( cat(3, G0_Fe3O4_s1, G0_Fe3O4_s2,G0_Fe3O4_s3, G0_Fe3O4_s4, G0_Fe3O4_l1)  ,[],3);  % stack the Go matrices along a dummy dimension, then take the min along that dimension.  Yes the [] is needed in the syntax for min() 

% now convert units to eV per Ga2O
G0_Fe3O4_ls = G0_Fe3O4_ls/(avo*q);   % eV/Ga2O molecule

% Now take Ptot and Xi into account.  
G0_Fe3O4_ls = G0_Fe3O4_ls + kB_eV*T.* log(X_i);

% set any that are zero becasue of masking to infintiy so it produces an
% obvious error that can be seen 
G0_Fe3O4_ls(G0_Fe3O4_ls==0) = Inf;
