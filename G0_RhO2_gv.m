function [G0_RhO2_gv] = G0_RhO2_gv(T, P_tot, X_i, P_units)
%
...
mask1 = (T>298) .* (T<=1500);      % gas

% solid
G0_RhO2_gv = mask1.*(169504.608 + 136.383579*T + 690360.000*T.^(-1)- 58.5760000*T.*log(T));

% now convert units to eV per Ga2O
G0_RhO2_gv = G0_RhO2_gv/(avo*q);   % eV/Ga2O molecule

% Now take Ptot and Xi into account.  
G0_RhO2_gv = G0_RhO2_gv + kB_eV*T.* ( log(P_tot/P_ref) + log(X_i));

% set any that are zero becasue of masking to infintiy so it produces an
% obvious error that can be seen 
G0_RhO2_gv(G0_RhO2_gv==0) = Inf;
