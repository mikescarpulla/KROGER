function [G0_TiO2_ls] = G0_TiO2_ls(T, P_tot, X_i, P_units)
%
...
G0_TiO2_s1 = zeros(size(T));
G0_TiO2_s2 = zeros(size(T));
G0_TiO2_liquid = zeros(size(T));
G0_TiO2_ls = zeros(size(T));
...
mask1 = (T>298) .* (T<=2130);      % solid 1
mask2 = (T>298) .* (T<=2100);      % solid 2    
mask3 = (T>298) .* (T<=2130);      % liquid. 

% solid1
G0_TiO2_s1 = mask1.*(-976986.650 +484.740378*T +1683920.51*T.^(-1) -67156778.7*T.^(-2) -77.8376214*T.*log(T));

% solid2 
G0_TiO2_s2 = mask2.*(-961316.130 +462.559573*T +881359.600*T.^(-1) -75.0358560*T.*log(T));

% liquid
G0_TiO2_liquid = mask3.*(-930962.650 +463.132871*T +1683920.51*T.^(-1) -67156778.7*T.^(-2) -77.8376214*T.*log(T));

% choose the min Go, thus automatically selecting the right phase.  This is
% written assuming
G0_TiO2_ls = min( cat(3,G0_TiO2_s1, G0_TiO2_s2, G0_TiO2_liquid),[],3);  % stack the two Go matrices along a dummy dimension, then take the min along that dimension.  Yes the [] is needed in the syntax for min() 

% now convert units to eV per Ga2O
G0_TiO2_ls = G0_TiO2_ls/(avo*q);   % eV/Ga2O molecule

% Now take Ptot and Xi into account.  
G0_TiO2_ls = G0_TiO2_ls + kB_eV*T.* ( log(P_tot/P_ref) + log(X_i));

% set any that are zero becasue of masking to infintiy so it produces an
% obvious error that can be seen 
G0_TiO2_ls(G0_TiO2_ls==0) = Inf;
