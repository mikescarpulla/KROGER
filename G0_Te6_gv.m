function [G0_Te6_gv] = G0_Te6_gv(T, P_tot, X_i, P_units)
...
mask1 = (T >= 298) & (T <= 1500);
G0_Te6_gv = G0_Te6_gv + mask1 .* (186092.459 + 400.039397*T - 1.764735460e-4*T.^2 + 125222.583*T.^(-1) + 4.663883209e-8*T.^3 - 6.326284409e-12*T.^4 - 132.869324*T.*log(T)); 


% now convert units to eV per Ga2O
G0_Te6_gv = G0_Te6_gv/(avo*q);   % eV/Ga2O molecule

% Now take Ptot and Xi into account.
G0_Te6_gv = G0_Te6_gv + kB_eV*T.*( log(P_tot/P_ref) + log(X_i));

% set any that are zero becasue of masking to infintiy so it produces an
% obvious error that can be seen
G0_Te6_gv(G0_Te6_gv==0) = Inf;
G0_Te6_gv(isnan(G0_Te6_gv)) = Inf;

end

% % View Data  Te6     Units:  T(K) P(atm) Energy(J) Quantity(mol) 
% % Name: Te6
% % 
% %   G(T) J/mol - 1 atm  
% % 
...
% % 
% % G1         1 186092.459               + 400.039397     T     - 1.764735460E-04 T^ 2 298 - 1500 
% % G1         1 + 125222.583     T^-1    + 4.663883209E-08 T^ 3 - 6.326284409E-12 T^ 4 298 - 1500 
% % G1         1 - 132.869324     T ln(T)                                               298 - 1500 
% % ____________ ________________________ ______________________ ______________________ __________ 
