function [G0_IrO2_gv] = G0_IrO2_gv(T, P_tot, X_i, P_units)
%
...
G0_IrO2_gv = G0_IrO2_gv + mask3.*(198199.809 + 116.765974*T - 1.101e-3*T.^(2) - 56.41*T.*log(T));

% now convert units to eV per Ga2O
G0_IrO2_gv = G0_IrO2_gv/(avo*q);   % eV/Ga2O molecule

% Now take Ptot and Xi into account.  
G0_IrO2_gv = G0_IrO2_gv + kB_eV*T.* ( log(P_tot/P_ref) + log(X_i));

% set any that are zero becasue of masking to infintiy so it produces an
% obvious error that can be seen 
G0_IrO2_gv(G0_IrO2_gv==0) = Inf;
