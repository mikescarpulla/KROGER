...
mask1 = (T>298) .* (T<=1100);      % gas/vapor
mask2 = (T>1100) .* (T<=4000);      

% gas/vapor
G0_CaO_gv = mask1.*(- 255475.808 - 15.1731427*T - 7.474858139e-3*T.^(2) + 13999.6597*T.^(-1) + 9.205931575e-8*T.^(3) + 1107.27182*log(T) - 25.7816397*T.*log(T));
G0_CaO_gv = G0_CaO_gv + mask2.*(152152.281 + 164.817017*T - 8.054003823e-05*T.^(2) - 12075583.2*T.^(-1) - 83128.2757*log(T) + 5947.37003*T.^(0.5) - 53.1457895*T.*log(T));
