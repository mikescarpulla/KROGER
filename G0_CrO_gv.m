function [G0_CrO_gv] = G0_CrO_gv(T, P_tot, X_i, P_units)
%
...
mask1 = (T>298) .* (T<=1000);      % gas/vapor
mask2 = (T>1000) .* (T<=3000);      

% liquid
G0_CrO_gv = mask1.*(172709.380 -32.9733595*T -6.276000000e-03*T.^(2) +31300.0000*T.^(-1) +9.556666667e-07*T.^(3) -30.0970000*T.*log(T));
G0_CrO_gv = G0_CrO_gv + mask2.*(166749.280 +37.4167452*T +1.479500000e-03*T.^(2) +873600.000*T.^(-1) +1.691666667e-07*T.^(3) -40.5550000*T.*log(T));

% now convert units to eV per Ga2O
G0_CrO_gv = G0_CrO_gv/(avo*q);   % eV/Ga2O molecule

% Now take Ptot and Xi into account.  
G0_CrO_gv = G0_CrO_gv + kB_eV*T.* ( log(P_tot/P_ref) + log(X_i));

% set any that are zero becasue of masking to infintiy so it produces an
% obvious error that can be seen 
G0_CrO_gv(G0_CrO_gv==0) = Inf;
...
% % % 
% % % % View Data  CrO     Units:  T(K) P(atm) Energy(J) Quantity(mol) 
...
% % % % 
% % % % L1         1 - 321531.193             + 328.167001     T     - 1.368335050E-03 T^ 2   298 - 1800  
% % % % L1         1 + 772673.200     T^-1    + 8912.35390     ln(T) - 2027.98876     T^0.5   298 - 1800  
% % % % L1         1 - 56.7277120     T ln(T)                                                 298 - 1800  
% % % % L1         2 - 312226.950             + 355.301302     T     - 62.7600000     T ln(T) 1800 - 2000 
% % % % G1         3 172709.380               - 32.9733595     T     - 6.276000000E-03 T^ 2   298 - 1000  
% % % % G1         3 + 31300.0000     T^-1    + 9.556666667E-07 T^ 3 - 30.0970000     T ln(T) 298 - 1000  
% % % % G1         4 166749.280               + 37.4167452     T     + 1.479500000E-03 T^ 2   1000 - 3000 
% % % % G1         4 + 873600.000     T^-1    + 1.691666667E-07 T^ 3 - 40.5550000     T ln(T) 1000 - 3000 
% % % % ____________ ________________________ ______________________ ________________________ ___________ 
% % % % 
