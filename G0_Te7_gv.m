function [G0_Te7_gv] = G0_Te7_gv(T, P_tot, X_i, P_units)
...
mask1 = (T >= 298) & (T <= 1500);
G0_Te7_gv = G0_Te7_gv + mask1 .* (206156.920 + 500.550310*T - 2.217230797e-4*T.^2 + 152503.466*T.^(-1) + 5.837882042e-8*T.^3 - 7.913375855e-12*T.^4 - 157.770050*T.*log(T)); 


% now convert units to eV per Ga2O
G0_Te7_gv = G0_Te7_gv/(avo*q);   % eV/Ga2O molecule

% Now take Ptot and Xi into account.
G0_Te7_gv = G0_Te7_gv + kB_eV*T.*( log(P_tot/P_ref) + log(X_i));

% set any that are zero becasue of masking to infintiy so it produces an
% obvious error that can be seen
G0_Te7_gv(G0_Te7_gv==0) = Inf;
G0_Te7_gv(isnan(G0_Te7_gv)) = Inf;

end

% % View Data  Te7     Units:  T(K) P(atm) Energy(J) Quantity(mol) 
% % Name: Te7
% % 
% %   G(T) J/mol - 1 atm  
% % 
...
% % 
% % G1         1 206156.920               + 500.550310     T     - 2.217230797E-04 T^ 2 298 - 1500 
% % G1         1 + 152503.466     T^-1    + 5.837882042E-08 T^ 3 - 7.913375855E-12 T^ 4 298 - 1500 
% % G1         1 - 157.770050     T ln(T)                                               298 - 1500 
% % ____________ ________________________ ______________________ ______________________ __________ 
% % 
