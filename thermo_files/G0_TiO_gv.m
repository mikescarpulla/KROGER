function [G0_TiO_gv] = G0_TiO_gv(T, P_tot, X_i, P_units)
%
...
% gas/vapor
G0_TiO_gv =  mask1.*(78198.9955 +125.670094*T +4.329207354E-03*T.^(2) -423001.463*T.^(-1) -3.836878386E-07*T.^(3) -7615.16143*log(T) -50.8736187*T.*log(T));
G0_TiO_gv = G0_TiO_gv + mask2.*(225627.296 -728.339114*T -3.858717941E-03*T.^(2) -416191.214*T.^(-1) -61603.3296*log(T) +15770.0655*T.^(0.5) +34.3386587*T.*log(T));


% now convert units to eV per Ga2O
G0_TiO_gv = G0_TiO_gv/(avo*q);   % eV/Ga2O molecule

% Now take Ptot and Xi into account.  
G0_TiO_gv = G0_TiO_gv + kB_eV*T.* ( log(P_tot/P_ref) + log(X_i));

% set any that are zero becasue of masking to infintiy so it produces an
% obvious error that can be seen 
G0_TiO_gv(G0_TiO_gv==0) = Inf;
