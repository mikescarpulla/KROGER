function [G0_SnO_ls] = G0_SnO_ls(T, P_tot, X_i, P_units)
%
...
mask1 = (T>298) .* (T<=1273);      % solid

% solid
G0_SnO_ls = G0_SnO_ls + mask1.*(-298331.317 + 215.499320*T -7.322000000E-03*T.^(2) -39.9572*T.*log(T));


% now convert units to eV per Ga2O
G0_SnO_ls = G0_SnO_ls/(avo*q);   % eV/Ga2O molecule

% Now take Ptot and Xi into account.  2 terms for gas/vapor and one for
% solid/liquid
G0_SnO_ls = G0_SnO_ls + kB_eV*T.* log(X_i) ;   

% set any that are zero becasue of masking to infintiy so it produces an
% obvious error that can be seen 
G0_SnO_ls(G0_SnO_ls==0) = Inf;
