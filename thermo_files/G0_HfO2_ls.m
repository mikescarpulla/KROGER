function [G0_HfO2_ls] = G0_HfO2_ls(T, P_tot, X_i, P_units)
%
...
G0_HfO2_s1 = zeros(size(T));
G0_HfO2_s2 = zeros(size(T));
G0_HfO2_l1 = zeros(size(T));
G0_HfO2_ls = zeros(size(T));
...
% solid1
G0_HfO2_s1 = mask1.*(-1170984.68 +433.620264*T -4.524996000E-03*T.^(2) +647055.600*T.^(-1) -72.1112400*T.*log(T) );

% solid2 - high pressure phase
G0_HfO2_s2 = G0_HfO2_s2 + mask2.*(-1160524.68 +428.318692*T -4.524996000E-03*T.^(2) +647055.600*T.^(-1) -72.1112400*T.*log(T)  );
G0_HfO2_s2 = G0_HfO2_s2 + mask3.*(-1214609.54 +725.217214*T -108.784000*T.*log(T));

% liquid
G0_HfO2_l1 = G0_HfO2_l1 + mask4.*(-1055924.68 +395.353045*T -4.524996000E-03*T.^(2) +647055.600*T.^(-1) -72.1112400*T.*log(T) );
G0_HfO2_l1 = G0_HfO2_l1 + mask5.*(-1110009.54 +692.251566*T -108.784000*T.*log(T) );

% choose the min Go, thus automatically selecting the right phase.  This is
% written assuming
G0_HfO2_ls = min( cat(3,G0_HfO2_s1, G0_HfO2_s2, G0_HfO2_l1),[],3);  % stack the two Go matrices along a dummy dimension, then take the min along that dimension.  Yes the [] is needed in the syntax for min() 
...
% % % 
% % % View Data  HfO2     Units:  T(K) P(atm) Energy(J) Quantity(mol) 
% % % Name: Hafnium Oxide
% % % 
% % %   G(T) J/mol - 1 atm  
% % % 
...
% % % 
% % % S1         1 - 1170984.68          + 433.620264     T       - 4.524996000E-03 T^ 2   298 - 1973  
% % % S1         1 + 647055.600     T^-1 - 72.1112400     T ln(T)                          298 - 1973  
% % % S2         2 - 1160524.68          + 428.318692     T       - 4.524996000E-03 T^ 2   298 - 1973  
% % % S2         2 + 647055.600     T^-1 - 72.1112400     T ln(T)                          298 - 1973  
% % % S2         3 - 1214609.54          + 725.217214     T       - 108.784000     T ln(T) 1973 - 3173 
% % % L1         4 - 1055924.68          + 395.353045     T       - 4.524996000E-03 T^ 2   298 - 1973  
% % % L1         4 + 647055.600     T^-1 - 72.1112400     T ln(T)                          298 - 1973  
% % % L1         5 - 1110009.54          + 692.251566     T       - 108.784000     T ln(T) 1973 - 3300 
% % % ____________ _____________________ ________________________ ________________________ ___________ 
% % % 
