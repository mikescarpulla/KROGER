function [G0_ZrO2_gv] = G0_ZrO2_gv(T, P_tot, X_i, P_units)
%
...
mask1 = (T>298) .* (T<=6000);      % gas/vapor

% gas/vapor
G0_ZrO2_gv = mask1.*(-309428.051 +124.420897*T +1196235.07*T.^(-1) +7.18939396*T.^(0.5) -64971632.0*T.^(-2) -58.1863462*T.*log(T));


% now convert units to eV per Ga2O
G0_ZrO2_gv = G0_ZrO2_gv/(avo*q);   % eV/Ga2O molecule

% Now take Ptot and Xi into account.  
G0_ZrO2_gv = G0_ZrO2_gv + kB_eV*T.* ( log(P_tot/P_ref) + log(X_i));

% set any that are zero becasue of masking to infintiy so it produces an
% obvious error that can be seen 
G0_ZrO2_gv(G0_ZrO2_gv==0) = Inf;
