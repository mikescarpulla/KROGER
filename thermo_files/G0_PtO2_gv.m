function [G0_PtO2_gv] = G0_PtO2_gv(T, P_tot, X_i, P_units)
%
...
mask1 = (T>298) .* (T<=2000);      % solid

% solid
G0_PtO2_gv = mask1.*(150226.046 + 118.879892*T - 1.046000000e-03*T.^(2)+ 575300.000*T.^(-1) - 55.4380000 *T.*log(T));

% now convert units to eV per Ga2O
G0_PtO2_gv = G0_PtO2_gv/(avo*q);   % eV/Ga2O molecule

% Now take Ptot and Xi into account.  
G0_PtO2_gv = G0_PtO2_gv + kB_eV*T.* ( log(P_tot/P_ref) + log(X_i));

% set any that are zero becasue of masking to infintiy so it produces an
% obvious error that can be seen 
G0_PtO2_gv(G0_PtO2_gv==0) = Inf;

end

% 
% View Data   PtO2     Units:  T(K) P(atm) Energy(J) Quantity(mol) 
% Name: Platnum Dioxide
% 
%   G(T) J/mol - 1 atm  
% 
%              G(T)                  G(T)                     G(T)                   T(K)       
...
% 
% G1         1 150226.046            + 118.879892     T       - 1.046000000E-03 T^ 2 298 - 2000 
% G1         1 + 575300.000     T^-1 - 55.4380000     T ln(T)                        298 - 2000 
% ____________ _____________________ ________________________ ______________________ __________ 
% 
