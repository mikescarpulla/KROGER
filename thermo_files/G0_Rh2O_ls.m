function [G0_Rh2O_ls] = G0_Rh2O_ls(T, P_tot, X_i, P_units)
%
...
mask1 = (T>298) .* (T<=973);      % solid

% solid
G0_Rh2O_ls = mask1.*(- 115627.889 + 330.304053*T - 1.353524000e-02*T.^(2)- 65.2285600 *T.*log(T));

% now convert units to eV per Ga2O
G0_Rh2O_ls = G0_Rh2O_ls/(avo*q);   % eV/Ga2O molecule

% Now take Ptot and Xi into account.  
G0_Rh2O_ls = G0_Rh2O_ls + kB_eV*T.* ( log(P_tot/P_ref) + log(X_i));

% set any that are zero becasue of masking to infintiy so it produces an
% obvious error that can be seen 
G0_Rh2O_ls(G0_Rh2O_ls==0) = Inf;

end


% View Data   Rh2O     Units:  T(K) P(atm) Energy(J) Quantity(mol) 
% Name: Rhodium(i) Oxide
% 
%   G(T) J/mol - 1 atm  
% 
%              G(T)                     G(T)               G(T)                   T(K)      
% ____________ ________________________ __________________ ______________________ _________ 
% 
% S1         1 - 115627.889             + 330.304053     T - 1.353524000E-02 T^ 2 298 - 973 
% S1         1 - 65.2285600     T ln(T)                                           298 - 973 
% ____________ ________________________ __________________ ______________________ _________ 
% 
