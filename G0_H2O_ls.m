function [G0_H2O_ls] = G0_H2O_ls(T, P_tot, X_i, P_units)
%
...
G0_H2O_ls = zeros(size(T));


% solid
mask1 = (T>0) .* (T<=273);      %logical array same size as T with ones where true and 0 where false
G0_H2O_ls = G0_H2O_ls + mask1.*(-303622.945 + 198.232091*T - 36.2460*T.*log(T));

% liquid
mask2 = (T>298) .* (T<=500);      % logical array same size as T with ones where true and 0 where false
G0_H2O_ls = G0_H2O_ls + mask2.*(-256638.942 - 1924378.83*T.^(-1) - 1118.62474*T - 0.760342801*T.^(2) +5.3188744e-4*T.^(3) - 2.059132015e-7*T.^(4) + 203.118982*T.*log(T));


% now convert units to eV per Ga2O
G0_H2O_ls = G0_H2O_ls/(avo*q);   % eV/Ga2O molecule

% Now take Ptot and Xi into account.  
G0_H2O_ls = G0_H2O_ls + kB_eV*T.* log(X_i);

% set any that are zero becasue of masking to infintiy so it produces an
% obvious error that can be seen 
G0_H2O_ls(G0_H2O_ls==0) = Inf;
